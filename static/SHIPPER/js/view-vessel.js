...
function applyFilters() {
  const typeFilter = document
    .querySelector(".filter-select:nth-child(1)")
    .value.toLowerCase();
  const statusFilter = document
    .querySelector(".filter-select:nth-child(2)")
    .value.toLowerCase();
  const originFilter = document
    .querySelector(".filter-select:nth-child(3)")
    .value.toLowerCase();
  const destinationFilter = document
    .querySelector(".filter-select:nth-child(4)")
    .value.toLowerCase();
...
document
  .querySelectorAll(".nav-item.has-submenu > .nav-link")
  .forEach((mainLink) => {
    ...
    mainLink.addEventListener("click", (e) => {
      ...
      const clickedChevronIcon =
        clickedMainLink.querySelector(".fa-chevron-down");

      const isClickedSubMenuAlreadyOpen =
        clickedSubMenu.classList.contains("open");
